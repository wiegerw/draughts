...
class MCTSEdge2Player(Player):
    def __init__(self, max_iterations: int, c: float = 1.0 / math.sqrt(2), simulate: Simulate=mcts_edge2.SimulatePieceCountEdge()):
        self.max_iterations = max_iterations
        self.c = c
        self.simulate = simulate
    ...
    def play(self, pos: Pos) -> Move:
        ...
        u = mcts_edge2.mcts(tree, self.c, self.max_iterations, self.simulate)
        return find_move(pos, u.state)

    def name(self) -> str:
        return f'MCTSEdge2Player(max_iterations={self.max_iterations},simulate={self.simulate})'


def main():
    player1 = MCTSEdge1Player(max_iterations=1000)
    player2 = MCTSEdge2Player(max_iterations=1000)
